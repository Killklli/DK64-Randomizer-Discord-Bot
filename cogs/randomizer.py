...
class Randomizer(commands.Cog, name="randomizer"):
    def __init__(self, bot):
        self.bot = bot
        self.randomizer_version = "DK64-Randomizer-Release"

    @commands.hybrid_command(
        name="presets",
        description="Lists the presets available.",
    )
    async def presets(self, context: Context):
        preset_data = []
    ...
    async def generate(self, context: Context, *, setting: str):
        preset_data = []
        with open(f"{self.randomizer_version}/static/presets/preset_files.json") as preset_files:
            file_contents = preset_files.read()
